...
AUG_FUNCS = ['filter'] #options are 'noise', and 'filter'. Will be run in order of list
AUG_OPTIONS = [{'frac':0.35}] #kwargs for the augmentation functions that are being used. if no options are provided, just uses default
AUG_METHOD = ['inplace'] #options are stack (which adds the traces to the end of the training stack) or inplace (which does augmentation on the training arr)
